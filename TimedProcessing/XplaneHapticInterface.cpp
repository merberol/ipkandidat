						...
						char *		outDesc)
{
	...
	// dataref strings required, get from config loader

	/*std::vector<std::string> DataRefString{
	...
	for (int i = 0; i < eventHandler->dataRefVector.size(); i++) {
		dataRefMap.emplace(eventHandler->dataRefVector[i], XPLMFindDataRef(eventHandler->dataRefVector[i].c_str()));
	}
